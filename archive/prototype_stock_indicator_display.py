...
common_length = min(len(ema), len(macd), len(macd_signal), len(dx), len(rsi), len(macd_hist))
...
buy_condition = (
    ...
    (dx_common >= 20) & 
    (rsi_common <= 40) #&
    #(c[-common_length:] > ema[-common_length:])
)
...
fig.add_trace(
    go.Scatter(x=dx_index, y=[20]*len(dx_index), mode='lines', name='Trend Threshold (20)',
